...
function Write-DryRunSummary {
    ...
    if ($Findings.HighPriorityFlags.Count -eq 0) {
        Write-Host "- None"
    }
    else {
...
if ($gpoEventCount -gt 0) {
    $findings.HighPriorityFlags += "Group Policy operational log has $gpoEventCount events in the incident window; check for new or failed policy application."
}
...
if ($perfEventCount -gt 0) {
    $findings.HighPriorityFlags += "Diagnostics-Performance log contains $perfEventCount relevant events; inspect for startup or logon degradation indicators."
}

$findings.SuggestedActions += "Compare this device evidence with at least two unaffected devices for differential analysis."
$findings.SuggestedActions += "If profile and GPO signals are elevated, prioritize rollback or disable of recent change set for Floor 6 pilot group."
$findings.SuggestedActions += "For Copilot restricted-file allegation, pair this output with cloud-side permission and audit logs from the security and governance team."
...
if ($DryRun) {
    Write-DryRunSummary -CaseFolder $caseFolder -CollectorStatus $collectorStatus -Findings $findings
}
else {
